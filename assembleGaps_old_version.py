...
def assembly(id_list, n):
    ...
    #             cnt=1
    #
    #         sf_fq="gap_reads/{0}_{1}.fastq".format(scaffold_id,cnt)
    #         cnt=cnt+1
    #         pre_id=scaffold_id
    #
...
def run_alignment_contig(id):
    ...
    if os.path.exists(sf)==True:
        cmd="bwa mem {0} {1} | " \
            "samtools view -S -h -b - | samtools sort - merged_contigs_alignment/{2}.sort " \
...
def run_alignment_reads(id):
    ...
    if os.path.exists(sf)==True:
        cmd="bwa mem {0} {1} | " \
            "samtools view -S -h -b - | samtools sort - gap_reads_alignment/{2}.sort " \
...
def align_unmapped_to_contigs(fa_list, n):
    ...
    cmd="bwa mem -a -t 10 {0} {1} {2} | samtools view -h -S -b - | " \
        "samtools sort - {3}".format(sf_ref, sf_left_reads, sf_right_reads, sf_algn)
    Popen(cmd, shell = True, stdout = PIPE).communicate()
    ...
    pool1 = ThreadPool(n)
    pool1.map(run_alignment_unmapped_reads, fa_list,bunch_size)
    pool1.close()
    pool1.join()
...
def run_collect_high_quality_unmap_to_contig_reads(id):
    ...
    cmd="bwa mem -T 15 {0} {1} | samtools view -S - > {2}".format(sf_contig,sf_reads,sf_algmt)
    Popen(cmd, shell = True, stdout = PIPE).communicate()
...
def run_filter(id):
    ...
    cmd="bwa mem -a -t 5 {0} {1} | samtools view -h -S -b - | " \
        "samtools sort - {2}.sort".format(fcontig, sf_reads,sf_sorted_bam)
    Popen(cmd, shell = True, stdout = PIPE).communicate()
...
def pick_full_constructed_contigs(bwa_min_score, sf_flank, n, fa_list, sf_picked):
    global m_flanks
    global BWA_MIN_SCORE
...
def pick_extended_contigs(n, fa_list, sf_picked):
    pool = ThreadPool(n)
    pool.map(run_pick_extended_contig, fa_list)
    pool.close()
    pool.join()
...
def validate_results(sf_fai, sf_gap_pos, n):
    fa_list=prepare_list(sf_fai, sf_gap_pos)
    pool = ThreadPool(n)
    pool.map(run_alignment_contig, fa_list)
    pool.map(run_alignment_reads, fa_list)
    pool.close()
    pool.join()
    ...
    cmd="cat chrom14_gap_seqs_header.txt merged_contigs_alignment.sam | samtools view -S -h -b - | " \
        "samtools sort - merged_contigs_alignment.sort"
    Popen(cmd, shell = True, stdout = PIPE).communicate()
    ...
    #     "samtools sort - merged_gap_reads_alignment.sort"
    # Popen(cmd, shell = True, stdout = PIPE).communicate()
...
if __name__ == "__main__":
    ...
    # print "Merge reads..."
    # merge_folder_list=[]
    # merge_folder_list.append("./../NA12878_high_coverage")
    # merge_folder_list.append("./../NA12878_long_insert")
    ...
    # merge_folder_list.append("./../long_insert_2k")
    # merge_folder_list.append("./../long_insert_30k")
    # merge_reads_v2(sys.argv[1],sys.argv[2], merge_folder_list, "gap_reads", int(sys.argv[3]))
    ...
    bwa_min_score=30
    sf_flank="./../flank_295.fa"
    sf_picked="picked_seqs.fa"

    # ###
    #print "First round assembly and merger..."
    #assembly(fa_list, n_jobs)
    #pick_full_constructed_contigs(bwa_min_score, sf_flank, n_jobs, fa_list, sf_picked)

    l_picked=get_already_picked(sf_picked)
    id_remain=[]
    for key in fa_list:
        if l_picked.has_key(key)==False:
    ...
    #
    # bam_list=[]
    # bam_list.append("./../NA12878_high_coverage/NA12878_pe_50x_300_2_scaffold_allpath_lg.sorted.bam")
    # bam_list.append("./../NA12878_mp_3k_35X/NA12878_mp_2_scaffold_allpath_lg.sort.bam")
    # # bam_list.append("./../short_insert/frag_2_scaffold.sort.bam")
    # # bam_list.append("./../long_insert_2k/shortjum_2_scaffold.sort.bam")
    # # bam_list.append("./../long_insert_30k/longjum_2_scaffold.sort.bam")
    # # #
    # print "Collect both unmapped reads..."
    # collect_both_unmapped_reads(bam_list, id_remain, n_jobs)
    #
    # ##
    # # print "Second round assembly and merger..."
    # assembly_given_list(id_remain,n_jobs)
    # run_contigs_merge(n_jobs, id_remain)


    # # print "Validating results..."
    #validate_results(sf_fai, sf_pos, n_jobs)
    ...
    # #
    # bwa_min_score=15
    # pick_full_constructed_contigs(bwa_min_score, sf_flank, n_jobs, id_remain, sf_picked)
    # #
    # pick_extended_contigs(n_jobs, id_remain, sf_picked)


    #validate_results(sf_fai, sf_pos, n_jobs)
    #run_contigs_assembly_test()
    #run_contigs_merge_test()
