...
self.m_both_unmap_ref={}
self.m_both_unmap_reads={}

def run_collect_both_unmapped(sf_bam): ###########################################################################################################
    sf_both_unmap=sf_bam+".both_unmapped.sam"
    cmd="{0} view -f 12 {1} > {2}".format(self.samtools_path, sf_bam, sf_both_unmap)
...
def run_alignment_unmapped_reads(id):
    ...
    for sctg in self.m_both_unmap_ref[id]:
        ...
        if output:
            ...
            for line in lines:
                ...
                if mapq!=255:
                    ...
                    if self.m_both_unmap_reads.has_key(s_read_id)==False:
                        print s_read_id, "Error happen!!!"
                        return
                    m_temp[s_read_id]=self.m_both_unmap_reads[s_read_id]
                    ...
                    if flag&8!=0 or g_read!=g_mate: #mate unmapped or not in same group
                        ...
                        if self.m_both_unmap_reads.has_key(s_mate_head)==False:
                            #error
                            print s_mate_head, "Error happen!!!!!!!"
                            return

                        m_temp[s_mate_head]=self.m_both_unmap_reads[s_mate_head]
        if error:
            succd=False
        del buffer
    except:
    ...
    with open(sf_fq,"a") as fout_fq:
        for shead in m_temp:
            fout_fq.write("@"+shead+"\n")
            fout_fq.write(self.m_both_unmap_reads[shead])
...
def align_unmapped_to_contigs(fa_list, n):
    ...
    with open(sf_ref,"w") as fout_ref:
        for key in fa_list:
            sf_ctg=self.working_folder+"velvet_temp/{0}/contigs.fa".format(key)
    ...
    pool1 = Pool(n)
    pool1.map(run_alignment_unmapped_reads, fa_list_hit, self.bunch_size)
    pool1.close()
    pool1.join()

    del self.m_both_unmap_reads
    del self.m_both_unmap_ref


def collect_both_unmapped_reads(self, bam_list, id_list, n):
    pool = ThreadPool(3)
    ...
    with open(sf_both_unmap) as fin_reads:
        for line in fin_reads:
            ...
            if cnt%4==0:
                self.m_both_unmap_reads[shead]=sread
                sread=""
                cnt=0
            cnt=cnt+1
    ...
    for key in self.m_both_unmap_reads:
        read_id=key[:-2]
        if key[-1]=="1":
            f_left.write("@"+read_id+"\n")
            f_left.write(self.m_both_unmap_reads[key])

            f_right.write("@"+read_id+"\n")
            f_right.write(self.m_both_unmap_reads[read_id+"_2"])
    f_left.close()
    f_right.close()
